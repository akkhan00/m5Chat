...
function saveLoginHistory(username, roomName) {
    ...
    history = history.filter(
        (item) => !(item.username === username && item.roomName === roomName),
    );
...
function selectFromHistory(username, roomName) {
    ...
    if (username.trim() && roomName.trim()) {
        const joinButton = elements.loginForm.querySelector(
            'button[type="submit"]',
        );
        if (joinButton) {
            joinButton.focus();
...
function updateUploadProgress(percentage) {
    const progressFill =
        elements.uploadProgress.querySelector(".progress-fill");
    const progressText =
    ...
    progressText.textContent =
        percentage === 100
            ? "Upload complete!"
            : `Uploading image... ${Math.round(percentage)}%`;
}
...
function createMessageElement(message, isOwnMessage = false) {
    ...
    if (message.reply_to && message.reply_content) {
        const replyUsername = message.reply_username
    ...
    const reactionsHTML = createReactionsHTML(
        message.reactions || {},
        message.id,
    );
...
function sendMessage(content) {
    if (!state.socket || !state.socket.connected) {
        alert(
            "Not connected to server. Please wait for connection to be restored.",
        );
        return;
    }
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const observer = new MutationObserver((mutations) => {
        mutations.forEach((mutation) => {
            if (
                mutation.target === elements.chatScreen &&
...
function updateMessageReactions(messageId, reactions) {
    ...
    if (reactionsContainer) {
        ...
        reactionsContainer.parentNode.replaceChild(
            tempDiv.firstChild,
            reactionsContainer,
        );
    }
...
function showReplyIndicator(content) {
    ...
    if (!replyIndicator) {
        ...
        const messageInputContainer =
            elements.messageForm.querySelector(".message-input-container") ||
        ...
        if (messageInputContainer === elements.messageForm) {
            ...
            elements.messageForm.insertBefore(
                replyIndicator,
